...
export const IntentUtil = Object.freeze({
	ALL: Object.keys(Intents).filter(k => isNaN(Number(k)) === true) as unknown as Intents,
	NONE: [] as unknown as Intents,
	DEFAULT: Object.keys(Intents).filter(k => isNaN(Number(k)) === true)
				.filter(intent => !["GUILD_MEMBERS", "GUILD_PRESENCES"].includes(intent)) as unknown as Intents
})