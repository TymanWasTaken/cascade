...
import {
  ...
  getUser,
  Message,
} from "https://deno.land/x/discordeno@10.2.0/mod.ts";
...
export class CascadeCommandHandler extends EventEmitter {
    ...
    "channel": (text, message) => {
      ...
      if (mentionMatch) {
        const channel = message.guild.channels.get(
          mentionMatch.groups?.id as string,
        );
  ...
  public async init() {
    ...
    const files = (await recursiveReaddir(this.options.commandDir)).map((f) =>
      join(".", f)
    ).filter(
      ...
      for (const p of prefixes) {
        for (const command of this.commands.values()) {
          for (const alias of command.options.aliases) {
            ...
            const afterPrefix = this.lstrip(
              this.lstrip(message.content, p + alias),
              " ",
            );
            return {
  ...
  public async parseArguments(
    message: CascadeMessage,
  ): Promise<CascadeCommandArgParse> {
    ...
    for (const arg of normal) {
      ...
      const parsed = await CascadeCommandHandler.types[arg.type](
        String(parse.args._[i]),
        message,
      );
      if (parsed == null) {
        return {
    ...
    for (const contentArg of content) {
      const parsed = await CascadeCommandHandler.types[contentArg.type](
        parse.content,
        message,
      );
  ...
  public async onMessage(message: CascadeMessage) {
    ...
    if (parse != null) {
      ...
      const inhibitorCheck = await this.client.inhibitorHandler.checkRun(
        msg,
        parse.command,
      );
      if (!inhibitorCheck.run) {
        this.emit("blocked", [
          inhibitorCheck.blockedReason,
          message,
          parse.command,
        ]);
        return;
  ...
  public isOwner(id: string) {
    ...
    if (Array.isArray(this.client.owners)) {
