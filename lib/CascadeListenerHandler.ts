...
export class CascadeListenerHandler extends EventEmitter {
  ...
  public async init() {
    ...
    const files = (await recursiveReaddir(this.options.listenerDir)).map((f) =>
      join(".", f)
    ).filter(
    ...
    for await (const listenerFile of files) {
      ...
      const listener: CascadeListener =
        new (await import("file://" + listenerPath)).default();
      if (!this.options.emitters[listener.options.emitter]) {
        throw new Error(`${listener.options.emitter} was not a set emitter!`);
      }
      this.options.emitters[listener.options.emitter].on(
      ...
      this.listeners.set(
        `${listener.options.emitter}-${listener.options.event}`,
        listener,
      );
    }
  ...
  public async onEvent(emitter: string, event: string, params?: any[]) {
    ...
    if (handler) {
      ...
      params = params?.map((v) => {
        try {
          return ("tts" in v)
            ? convertMessage(v, this.client as CascadeClient, null)
            : v;
        } catch {
          return v;
        }
